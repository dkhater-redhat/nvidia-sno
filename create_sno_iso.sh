...
# install-config.yaml already generated by configure-sno.sh, just substitute SSH_KEY and PULL_SECRET
envsubst < install-config.yaml > install-config.yaml.tmp && mv install-config.yaml.tmp install-config.yaml
